...
from .LISDE import LISDE
# from .scaler import LScaler
from .score_fn import EpsilonTheta
from .distribution_output import score_fn_output



class LISDETrainingNetwork(nn.Module):
    @validated()
    def __init__(
        self,
        input_size: int,
        ...
        **kwargs,
        ...
        self.sde = LISDE(self.model, sigma=sigma, eps=self.eps)

        
        self.distr_output = score_fn_output(
            input_size=target_dim, cond_size=conditioning_length
        )
        ...
        if self.scaling == 1:
            ...
            print('using mean scaler')
        elif self.scaling == 0:
            self.scaler = NOPScaler(keepdim=True)
            print('using no scaler')
        else:
    ...
    def unroll(
        ...
        if std == None:
            ...
            lags_scaled = (lags - scale.unsqueeze(-1)) / std.unsqueeze(-1)
        ...
        # 这里是把过去的特征（过去1小时，24小时，7天同一时间），index的embedding以及时间的feat加到一起
    ...
    def unroll_encoder(
        ...
        lags = self.get_lagged_subsequences(
            ...
            indices=self.lags_seq, # 往前看的点，比如1，24，168，即过去1小时，24小时同一时间，一周同一时间
            subsequences_length=subsequences_length,
        )

        # scale is computed on the context length last units of the past target
        # scale shape is (batch_size, 1, target_dim)
        if self.scaling == 1 or self.scaling == 0:
            _, scale = self.scaler(
        ...
        return outputs, states, scale, lags_scaled, inputs, std
    ...
    def forward(
        ...
        # as well
        rnn_outputs, _, scale, _, _, std = self.unroll_encoder(
        ...
        if self.scaling == 1 or self.scaling == 0:
            ...
            target = (target - scale) / std
        
        likelihoods = self.sde(target.reshape(B*T, 1, -1), distr_args.reshape(B*T, 1, -1))
...
class LISDEPredictionNetwork(LISDETrainingNetwork):
    def __init__(self, 
    ...
    def sampling_decoder(
        ...
        for k in range(self.prediction_length):
            ...
            new_samples = self.sde.sample(sampling_shape, 
                                          num_steps=self.diff_steps,
                                          device=distr_args.device,
                                          denoise=self.denoise,
                                          clamp=self.clamp,
                                          clamp_val=self.clamp_val,
                                          cond=distr_args)
            if self.scaling == 1 or self.scaling == 0:
                new_samples *= repeated_scale
            else:
                new_samples = (new_samples * repeated_std)  + repeated_scale
    ...
    def forward(
        ...
        t_out, begin_states, scale, _, _, std = self.unroll_encoder(
            past_time_feat=past_time_feat,
            past_target_cdf=past_target_cdf,
            past_observed_values=past_observed_values,
            past_is_pad=past_is_pad,
        ...
        bs = t_out.shape[0]
        sampling_shape = [bs * self.num_parallel_samples, 1, self.target_dim]
