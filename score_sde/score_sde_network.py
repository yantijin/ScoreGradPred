...
class ScoreGradTrainingNetwork(nn.Module):
    @validated()
    def __init__(
        self,
        input_size: int,
    ...
    def forward(
        ...
        if self.scaling:
            target /= scale
        likelihoods = self.loss_fn(self.model, target.reshape(B*T, 1, -1), distr_args.reshape(B*T, 1, -1)).unsqueeze(-1)
...
class ScoreGradPredictionNetwork(ScoreGradTrainingNetwork):
    def __init__(self, num_parallel_samples: int, config, **kwargs) -> None:
        super().__init__(**kwargs)
        self.config = config
        self.num_parallel_samples = num_parallel_samples
        if self.config.weight_decay:
            self.ema = ExponentialMovingAverage(self.model.parameters(), self.config.weight_decay)
    ...
    def sampling_decoder(
        ...
        sampling_fn = get_sampling_fn(self.config, self.sde, sampling_shape, eps=self.sampling_eps)
        ...
        for k in range(self.prediction_length):
            ...
            if self.config.weight_decay:
                ...
                new_samples, _ = sampling_fn(self.model, distr_args)
    ...
    def forward(
        ...
        t_out, begin_states, scale, _, _ = self.unroll_encoder(
            past_time_feat=past_time_feat,
            past_target_cdf=past_target_cdf,
            past_observed_values=past_observed_values,
            past_is_pad=past_is_pad,
